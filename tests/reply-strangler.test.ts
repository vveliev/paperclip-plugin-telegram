...
// GIF-155: "Strangle, don't sweep." src/reply.ts is now the one place new
...
//
// The 116 existing call sites across this list are not touched by GIF-155 --
...
describe("no new direct callers of telegram-api's sendMessage (GIF-155)", () => {
      ...
      if (callsSendMessage(src)) {
        expect(
          ...
          `${file} calls telegram-api's sendMessage directly but is not on the allowlist. ` +
            ...
            "reaching for the raw transport (GIF-155). If this file is intentionally " +
