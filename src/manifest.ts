...
import { SECRET_REF_CONFIG_KEYS } from "./config.js";
...
const manifest: PaperclipPluginManifestV1 = {
  ...
  instanceConfigSchema: {
    ...
    properties: Object.fromEntries(
      SECRET_REF_CONFIG_KEYS.map((key) => [
        key,
        {
          // Secret refs accept BOTH shapes on purpose.
          //
          // Older hosts store a bare secret UUID string. Current hosts
          // validate config against this schema AND separately enforce their
          // own secret-ref rule requiring an object:
          // { type: "secret_ref", secretId, version? }. With `type: "string"`
          // alone the two are mutually unsatisfiable — the object fails this
          // schema ("must be string") and the string fails the host ('must
          // use { type: "secret_ref", ... }') — so the plugin cannot be
