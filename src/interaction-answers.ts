...
type FreshInteraction = { id: string; status: string; kind: string };

async function fetchFreshInteraction(
  ctx: PluginContext,
  baseUrl: string,
  issueId: string,
  interactionId: string,
...
export async function resolveInteractionAnswerCallback(
  ...
  const fresh = await fetchFreshInteraction(ctx, baseUrl, parked.issueId, parked.interactionId, boardApiToken);
...
export async function finalizeReplyRejection(
  ...
  try {
    const fresh = await fetchFreshInteraction(ctx, baseUrl, mapping.issueId, mapping.entityId, boardApiToken);
    if (!fresh || fresh.status !== "pending") {
      const status = fresh?.status ?? "gone";
